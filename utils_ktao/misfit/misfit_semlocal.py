...
#
  ...
  def setup_event_from_FORCESOLUTION(self, input_file):
    ...
    tmp = lines[0].split()
    event_id = str(tmp[0]).strip()
    ref_time = UTCDateTime(str(tmp[1]).strip())
    ...
    event = {
        ...
        'xyz':np.array([x,y,z]),
        't0':ref_time + time_shift, 'tau':tau,
...
#
  ...
  def setup_station(self,
    ...
    lat1 = np.array([ float(l[4]) for l in lines])
    lon1 = np.array([ float(l[5]) for l in lines])
    ...
    if REF_ellps == "WGS84":
      ...
      grd_z2 = fh.variables['z'][:]
      grd_lon2, grd_lat2 = np.meshgrid(grd_lon1, grd_lat1, indexing='xy')
      topo = interpolate.griddata((grd_lon2.flatten(), grd_lat2.flatten()), grd_z2.flatten(), (lon1, lat1), method='cubic')
    elif REF_ellps == "sphere":
      topo = np.zeros(lat1.shape)
    else:
    ...
    for net_sta_loc in metadata:
      ...
      lats = None; lats = [ x['latitude'] for x in channel ]
      lons = None; lons = [ x['longitude'] for x in channel ]
      eles = None; eles = [ x['elevation'] for x in channel ]
      deps = None; deps = [ x['depth'] for x in channel ]
      ...
      event_alt = event['altitude']
      if event_alt > 0.0:
        event_alt = 0.0
      arrivals = taup_model.get_travel_times(
          source_depth_in_km=-1.0*event_alt/1000.0,
...
#
  ...
  def read_obs_syn(self,
    ...
    for station_id in station_dict:
      ...
      # convert from REF_ENU to local ENU
...
#
  ...
  def output_adj_REF_ENU(self, 
    ...
    for station_id in station_dict:
      ...
      # now adjoint source is in local ENU coordinate, 
      # we need to conver it to REF_ENU
      rotmat = meta['rotmat_local_to_ref']
      # from REF_ENU to local ENU
      adj = np.dot(rotmat, station['dchi_du'])
