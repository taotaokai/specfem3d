...
stalt = interpolate.griddata((grd_lons, grd_lats), grd_alts, (stlo, stla), method='cubic')
...
with open(out_file, "w") as f:
  for i in range(len(stlo)):
    ...
    stnm_chan = "%s.%s"%(stnm[i], stch[i])
    f.write("%-10s  %-10s  %+15.5e  %+15.5e  0.0  %+15.5e\n"%(netwk[i], stnm_chan, nn[i], ee[i], uu[i]) )
