...
vp = np.array([float(x[1]) for x in lines])*1000 # km/s -> m/s
vs = np.array([float(x[2]) for x in lines])*1000 # km/s -> m/s
rho = np.array([float(x[3]) for x in lines])*1000 # g/cm^3 -> kg/m^3
...
for iproc in range(nproc):
  ...
  #FIXME should use the depth beneath the topography here, instead of the ellipsoid surface
